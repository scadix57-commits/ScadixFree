...
namespace Scadix.AxamlDom
{
	...
	static class MarkupExtensionParser
	{
		public static XamlObject Parse(string text, XamlObject parent, XmlAttribute attribute)
		{
			...
			if (!typeName.EndsWith("Extension")) extensionType = typeResolver.Resolve(typeName + "Extension");
			...
			if (ctors.Count < 1 && typeof(Avalonia.Data.BindingBase).IsAssignableFrom(extensionType))
				ctors = extensionType.GetConstructors().Where(c => c.GetParameters().Length == 0).ToList();
			if (ctors.Count < 1)
			...
			object instance;
			// Special handling for Avalonia Binding - it's not a MarkupExtension
			if (typeof(Avalonia.Data.BindingBase).IsAssignableFrom(extensionType)) {
				instance = Activator.CreateInstance(extensionType);
				var pathProperty = extensionType.GetProperty("Path");

				// Handle positional arg as Path
				if (positionalArgs.Count > 0 && pathProperty != null) {
					try {
						pathProperty.SetValue(instance, XamlParser.CreateObjectFromAttributeText(positionalArgs[0], pathProperty.PropertyType, parent));
					} catch { }
					positionalArgs.Clear();
				}
			} else if (mappingToProperties) {
