...
namespace Scadix.AxamlDom
{
	...
	public sealed class XamlProperty
	{
		...
		void SetPropertyValue(XamlPropertyValue value)
		{
			...
			if (xamlObject != null && (xamlObject.IsMarkupExtension || xamlObject.Instance is BindingBase))
			...
			if (PropertyValue != null) {
				try {
					...
					if (propertyInfo.DependencyProperty == DesignTimeProperties.DesignWidthProperty) {
						var widthProperty = this.ParentObject.Properties.FirstOrDefault(x => x.DependencyProperty == Control.WidthProperty);
						if (widthProperty == null || !widthProperty.IsSet)
							((Control) this.ParentObject.Instance).Width = (double) ValueOnInstance;
					}

					if (propertyInfo.DependencyProperty == DesignTimeProperties.DesignHeightProperty) {
						var heightProperty = this.ParentObject.Properties.FirstOrDefault(x => x.DependencyProperty == Control.HeightProperty);
						if (heightProperty == null || !heightProperty.IsSet)
							((Control)this.ParentObject.Instance).Height = (double)ValueOnInstance;
					}
				}
				catch (Exception ex) {
