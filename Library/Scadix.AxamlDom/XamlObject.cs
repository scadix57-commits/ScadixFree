...
namespace Scadix.AxamlDom
{
	...
	public sealed class XamlObject : XamlPropertyValue
	{
		...
		public bool IsMarkupExtension {
			get { 
				return instance is Avalonia.Markup.Xaml.MarkupExtension
				    || instance is Avalonia.Data.Binding
				    || instance is Avalonia.Data.BindingBase
                    || Instance is RelativeSource || ElementType.Name.EndsWith("Extension"); ;
			}
		}
...
//				return
		...
		object ProvideValue()
		{
			...
			if (this.ParentObject != null && this.ParentObject.ElementType == typeof (Setter) && this.ElementType == typeof(DynamicResourceExtension))
				return Instance;
			return (Instance as Avalonia.Markup.Xaml.MarkupExtension).ProvideValue(ServiceProvider);
		}
	...
	class BindingWrapper : MarkupExtensionWrapper
	{
		...
		public override object ProvideValue()
		{
			...
			// Handle ElementName binding - resolve the name to actual object via NameScope
			if (target is Binding b && !string.IsNullOrEmpty(b.ElementName))
			{
				var nameResolver = XamlObject.ServiceProvider?.GetService(typeof(IXamlNameResolver)) as IXamlNameResolver;
				if (nameResolver != null)
				{
					var source = nameResolver.Resolve(b.ElementName);
					if (source != null)
					{
						return new Binding
						{
