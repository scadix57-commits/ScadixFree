...
namespace Scadix.AxamlDom
{
	...
	public class XamlTypeFinder : ICloneable
	{
		...
		sealed class XamlNamespace
		{
            ...
            internal string UsingClrNamespace;
			...
			internal XamlNamespace Clone()
			{
                ...
                copy.UsingClrNamespace = UsingClrNamespace;
		...
		XamlNamespace ParseNamespace(string xmlNamespace)
		{
            ...
            Debug.Assert(name.StartsWith("using:", StringComparison.Ordinal) || name.StartsWith("clr-namespace:", StringComparison.Ordinal));
            
            if (name.StartsWith("using:", StringComparison.Ordinal)) name = name.Substring("using:".Length);
            else if (name.StartsWith("clr-namespace:", StringComparison.Ordinal)) name = name.Substring("clr-namespace:".Length);
			...
			if (pos < 0) {
				...
				namespaceName = name.Substring(0, pos).Trim();
            ...
            {
                ns.UsingClrNamespace = namespaceName;
                foreach (var registeredAsm in _registeredAssemblies.ToList())
                {
                    AddMappingToNamespace(ns, new AssemblyNamespaceMapping(registeredAsm, namespaceName));
                }
            }
            return ns;
		}
        ...
        /// <summary>
        /// Directly registers a mapping between an XML namespace (e.g. "using:MyApp.ViewModels")
        /// and a CLR namespace inside <paramref name="assembly"/>.
        /// Use this to pre-register project namespaces before XAML parsing begins.
        /// </summary>
        public void RegisterNamespaceMapping(string xmlNamespace, string clrNamespace, Assembly assembly)
        {
            if (string.IsNullOrEmpty(xmlNamespace) || string.IsNullOrEmpty(clrNamespace) || assembly == null)
                return;

            // Ensure the assembly is in _registeredAssemblies
            var assemblyName = assembly.GetName().Name;
            if (!_registeredAssemblies.Any(a => string.Equals(a.GetName().Name, assemblyName, StringComparison.OrdinalIgnoreCase)))
                _registeredAssemblies.Add(assembly);

            // Get or create the XamlNamespace entry
            if (!namespaces.TryGetValue(xmlNamespace, out var ns))
            {
                ns = new XamlNamespace(null, xmlNamespace);
                ns.UsingClrNamespace = clrNamespace;
                namespaces[xmlNamespace] = ns;
            }

            var mapping = new AssemblyNamespaceMapping(assembly, clrNamespace);
            if (!ns.ClrNamespaces.Contains(mapping))
        ...
        public void RegisterAssembly(Assembly assembly)
        {
            ...
            var assemblyNameTest = assembly.GetName().Name;

            // Check if already registered
            if (RegisteredAssemblies.Any(a => a.GetName().Name == assemblyNameTest))
                return;
            ...
            {
                if (ns.UsingClrNamespace != null)
                    AddMappingToNamespace(ns, new AssemblyNamespaceMapping(assembly, ns.UsingClrNamespace));
            }
            ...
            foreach (XmlnsDefinitionAttribute xmlnsDef in assembly.GetCustomAttributes(typeof(XmlnsDefinitionAttribute), true))
            {
                ...
                var asmNameProp = typeof(XmlnsDefinitionAttribute).GetProperty("AssemblyName");
                if (asmNameProp != null)
                    assemblyName = asmNameProp.GetValue(xmlnsDef) as string;
		...
		static class WpfTypeFinder
		{
			...
			static WpfTypeFinder()
			{
                ...
                Instance.RegisterAssembly(typeof(Grid).Assembly);      // Avalonia.Controls
                Instance.RegisterAssembly(typeof(Avalonia.Markup.Xaml.MarkupExtensions.StaticResourceExtension).Assembly);
                Instance.RegisterAssembly(typeof(Avalonia.Data.BindingBase).Assembly);
