	...
	public class CreateComponentTool : ITool
	{
				...
				try {
                    if (container.Parent != null)
                    {
                        ...
                        for (var index = 0; index < positions.Count; index++)
                            positions[index] = new Rect(new Point(positions[index].X + rel.Value.X, positions[index].Y + rel.Value.Y), positions[index].Size);
                    }
                }
				catch(Exception)
				{ }
		...
		void OnMouseDown(object sender, PointerPressedEventArgs e)
		{
			...
			if (props.IsLeftButtonPressed && MouseGestureBase.IsOnlyButtonPressed(e, MouseButton.Left)) {
				e.Handled = true;
				IDesignPanel designPanel = (IDesignPanel)sender;
				DesignPanelHitTestResult result = designPanel.HitTest(e.GetPosition(designPanel as Avalonia.Visual), false, true, HitTestType.Default);
				if (result.ModelHit != null) {
