...
namespace Scadix.AxamlDesigner.Services
{
	...
	public class DefaultSelectionService : ISelectionService, INotifyPropertyChanged
	{
		...
		// Store last known key modifiers - updated by callers via SetKeyModifiers
		private KeyModifiers _lastKeyModifiers = KeyModifiers.None;
		
		public void SetKeyModifiers(KeyModifiers modifiers)
		{
			_lastKeyModifiers = modifiers;
		}
		...
		public virtual void SetSelectedComponents(ICollection<DesignItem> components, SelectionTypes selectionType)
		{
			...
			if (selectionType == SelectionTypes.Auto) {
				// KeyboardDevice.Instance not available in Avalonia 12 - use stored modifiers
				var modifiers = _lastKeyModifiers;
