...
using DragListener = Scadix.AxamlDesigner.Controls.DragListener;
using Avalonia;
			...
			public Point TranslatedPoint
			{
				get {
					if (_shape.RenderedGeometry.Transform == null)
						return Point;
					return _shape.RenderedGeometry.Transform.Value.Transform(Point);
				}
				set {
					if (_shape.RenderedGeometry.Transform == null)
						Point = value;
					else
						Point = _shape.RenderedGeometry.Transform.Value.Invert().Transform(value);
				}
			}
...
#pragma warning restore 0414
		...
		protected virtual Control[] BuildMenu(PathPoint pathpoint)
		{
			...
			if (pathpoint.TargetPathPoint == null && (pathpoint.Object is LineSegment || pathpoint.Object is PolyLineSegment || pathpoint.Object is BezierSegment || pathpoint.Object is QuadraticBezierSegment || pathpoint.Object is ArcSegment)) {
				...
				menuItem.Click += (s, e) => ConvertPart(((AvaloniaObject)s).TryFindParent<PathThumb>(), PathPartConvertType.insertPoint);
				menuList.Add(menuItem);
				menuItem = new MenuItem() { Header = "to Line Segment" };
				menuItem.Click += (s, e) => ConvertPart(((AvaloniaObject)s).TryFindParent<PathThumb>(), PathPartConvertType.ToLineSegment);
				menuList.Add(menuItem);
				menuItem = new MenuItem() {Header = "to Bezier Segment" };
				menuItem.Click += (s, e) => ConvertPart(((AvaloniaObject)s).TryFindParent<PathThumb>(), PathPartConvertType.ToBezierSegment);
				menuList.Add(menuItem);
				menuItem = new MenuItem() {Header = "to Quadric Bezier Segment" };
				menuItem.Click += (s, e) => ConvertPart(((AvaloniaObject)s).TryFindParent<PathThumb>(), PathPartConvertType.ToQuadricBezierSegment);
				menuList.Add(menuItem);
				menuItem = new MenuItem() { Header = "to Arc Segment" };
				menuItem.Click += (s, e) => ConvertPart(((AvaloniaObject)s).TryFindParent<PathThumb>(), PathPartConvertType.ToArcSegment);
				menuList.Add(menuItem);
				menuList.Add(new Separator());
				menuItem = new MenuItem() { Header = "is Stroked", IsChecked = ((PathSegment)pathpoint.Object).IsStroked };
				menuItem.Click += (s, e) => ChangeIsStroked(((AvaloniaObject)s).TryFindParent<PathThumb>(), (MenuItem)s);
				menuList.Add(menuItem);
			...
			if (pathpoint.Object is PathFigure) {
				...
				menuItem.Click += (s, e) => ChangeIsClosed(((AvaloniaObject)s).TryFindParent<PathThumb>(), (MenuItem)s);
				menuList.Add(menuItem);
			}
		...
		protected virtual PathThumb CreateThumb(PlacementAlignment alignment, Cursor cursor, int index, PathPoint pathpoint, Transform transform)
		{
			...
			designerThumb.InnerRenderTransform = new MatrixTransform(transform.Value.Invert());
			...
			if (pathpoint.TargetPathPoint != null) {
				...
				designerThumb.Bind(PathThumb.RelativeToPointProperty, bnd);
			}
		...
		protected override void OnInitialized()
		{
			...
			for (int i = 0; i < pathPoints.Count; i++) {
				CreateThumb(PlacementAlignment.BottomRight, new Cursor(StandardCursorType.Cross), i, pathPoints[i], transform);
			}
		...
		protected List<PathPoint> GetPoints(DesignItem designItem)
		{
			...
			AddGeometryPoints(retVal, ((Path)designItem.View).Data, ((Path)designItem.View), () => designItem.Properties["Data"].SetValue(((Avalonia.Controls.Shapes.Path)designItem.View).Data.ToString()));
			...
			if (geometry is CombinedGeometry) {
				...
				if (d != null) {
					if (!d.ParentProperty.IsCollection)
						((AvaloniaObject)d.ParentProperty.DesignItem.Component).SetValue(d.ParentProperty.DependencyProperty, pg);
					else {
						var collection = ((AvaloniaObject)d.ParentProperty.DesignItem.Component) as GeometryGroup;
