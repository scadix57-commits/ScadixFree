...
namespace Scadix.AxamlDesigner.Extensions
{
	public class SnaplinePlacementBehavior : RasterPlacementBehavior
	{
		...
		public static readonly AvaloniaProperty DisableSnaplinesProperty =
		...
		// Track last key modifiers from placement operations
		...
		public override void BeforeSetPosition(PlacementOperation operation)
		{
			...
			surface.Children.Clear();
			if ((_lastKeyModifiers & KeyModifiers.Control) != 0) return;
			
			// Compute union of all placed item bounds
			Rect bounds = default(Rect);
			bool boundsInit = false;
			foreach (var item in operation.PlacedItems) {
				if (!boundsInit) { bounds = item.Bounds; boundsInit = true; }
				else bounds = bounds.Union(item.Bounds);
			}
			...
			if (Snap(horizontalInput, horizontalMap, SnaplineAccuracy, out drawLines, out delta))
			{
				...
				if (operation.Type == PlacementType.Resize)
				{
					...
					{
						double newY = bounds.Y + delta;
						double newH = Math.Max(0, bounds.Height - delta);
						if (operation.CurrentContainer.Services.GetService<OptionService>().SnaplinePlacementRoundValues) {
							newY = Math.Round(newY);
							newH = Math.Round(newH);
						}
						bounds = new Rect(bounds.X, newY, bounds.Width, newH);
					}
					else
					{
						double newH = Math.Max(0, bounds.Height + delta);
						...
						bounds = new Rect(bounds.X, bounds.Y, bounds.Width, newH);
					}
					info.Bounds = bounds;
				}
				else
				{
					...
					{
						var r = item.Bounds;
						double newY = r.Y + delta;
						if (operation.CurrentContainer.Services.GetService<OptionService>().SnaplinePlacementRoundValues) {
							newY = Math.Round(newY);
						}
						item.Bounds = new Rect(r.X, newY, r.Width, r.Height);
					}
				}
			...
			if (Snap(verticalInput, verticalMap, SnaplineAccuracy, out drawLines, out delta))
			{
				...
				if (operation.Type == PlacementType.Resize)
				{
					...
					{
						double newX = bounds.X + delta;
						double newW = Math.Max(0, bounds.Width - delta);
						if (operation.CurrentContainer.Services.GetService<OptionService>().SnaplinePlacementRoundValues) {
							newX = Math.Round(newX);
							newW = Math.Round(newW);
						}
						bounds = new Rect(newX, bounds.Y, newW, bounds.Height);
					}
					else
					{
						double newW = Math.Max(0, bounds.Width + delta);
						...
						bounds = new Rect(bounds.X, bounds.Y, newW, bounds.Height);
					}
					info.Bounds = bounds;
				}
				else
				{
					...
					{
						var r = item.Bounds;
						double newX = r.X + delta;
						if (operation.CurrentContainer.Services.GetService<OptionService>().SnaplinePlacementRoundValues) {
							newX = Math.Round(newX);
						}
						item.Bounds = new Rect(newX, r.Y, r.Width, r.Height);
					}
				}
		...
		void AddLines(Rect r, double inflate, bool requireOverlap, List<Snapline> h, List<Snapline> v, PlacementAlignment? filter)
		{
			if (r != default(Rect))
			{
				Rect r2 = r.Inflate(new Thickness(Math.Abs(inflate)));
				if (inflate < 0)
					r2 = new Rect(r.X - inflate, r.Y - inflate, Math.Max(0, r.Width + inflate * 2), Math.Max(0, r.Height + inflate * 2));
		...
		void AddBaseline(DesignItem item, Rect bounds, List<Snapline> list)
		{
			...
			if (baseline.HasValue) {
				var tp = item.View.TranslatePoint(new Point(0, baseline.Value), ExtendedItem.View);
				if (tp.HasValue)
					list.Add(new Snapline() { Group = 1, Offset = tp.Value.Y, Start = bounds.Left, End = bounds.Right });
			}
		}
		...
		static double? GetBaseline(Control element) {
			...
			if (textBox != null) {
				// GetRectFromCharacterIndex not available in Avalonia - return null
				return null;
			}
			var textBlock = element as TextBlock;
			if (textBlock != null) {
				var tp = textBlock.TranslatePoint(new Point(0, textBlock.Bounds.Height), element);
				return tp?.Y;
			}
