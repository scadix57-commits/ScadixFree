...
namespace Scadix.AxamlDesign
{
	...
	public abstract class DesignItem : INotifyPropertyChanged
	{
		...
		{
			var manager = this.Services.GetService<ExtensionManager>();
        ...
        public Transform GetCompleteAppliedTransformationToView()
        {
            ...
            while (v != null)
            {
                ...
                if (fe is LayoutTransformControl ltc && ltc.LayoutTransform != null)
                {
                    if (ltc.LayoutTransform is Transform t)
                        retVal.Children.Add(t);
                    else if (ltc.LayoutTransform is TransformOperations ops)
                        retVal.Children.Add(new MatrixTransform(ops.Value));
                }

                if (fe != null && fe.RenderTransform != null)
                {
                    if (fe.RenderTransform is Transform t2)
                        retVal.Children.Add(t2);
                    else if (fe.RenderTransform is TransformOperations ops2)
                        retVal.Children.Add(new MatrixTransform(ops2.Value));
                }
